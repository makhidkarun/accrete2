...
extern double stellar_luminosity_ratio, stellar_mass_ratio, ecosphere_radius, age;
...
int gas_giant;
{
     ...
     temp = temp * pow1_4(ecosphere_radius / orbital_radius);
     if (gas_giant)
	  return(temp * 1.2);
     else 
	  return(temp * 5.5);
...
double day_length(mass, radius, eccentricity, density,
		  orbital_radius, orbital_period, giant)
double mass, radius, eccentricity, density, orbital_radius, orbital_period;
int giant;
{
	...
	spin_resonance_factor, year_in_hours, day_in_hours;
     ...
     year_in_hours = orbital_period * 24.0;
     ...
     change_in_angular_velocity = CHANGE_IN_EARTH_ANG_VEL *
				  ...
				  pow(stellar_mass_ratio, 2.0) *
				  (1.0 / pow(orbital_radius, 6.0));
...
/*  molecule or atom.  The velocity returned is in m/sec.		    */
/*--------------------------------------------------------------------------*/
...
double molecular_weight, orbital_radius;
{
     ...
     exospheric_temp = EARTH_EXOSPHERE_TEMP / pow2(orbital_radius);
...
int zone, greenhouse_effect;
{
     ...
     if (velocity_ratio >= GAS_RETENTION_THRESHOLD)
     {
	  ...
	  mass_in_earth_units = mass * SUN_MASS_IN_EARTH_MASSES;
...
{
     return(sqrt(ecosphere_radius / orbital_radius)
